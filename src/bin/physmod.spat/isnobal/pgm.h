...
extern	void	assign_buffers (int masked, int output);
...
extern	void	copy_image     (char *tempfile, int nbands, fpixel_t * buf,
				int fdo);
extern	void 	e_m_image      (int step, char * tempfile);
extern	bool_t	extract_data   (bool_t first_step, bool_t sun_up[]);
extern	void	headers        (void);
extern	void	isnobal        (int out_step, int got_opt_F);
...
extern	void 	snow_image     (int step, char * tempfile);
...
extern	int		nlines;		/* # lines in snow output image	     */
