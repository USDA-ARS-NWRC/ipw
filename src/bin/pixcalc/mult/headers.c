...
head1()
{
 ...
 /*
  * convert lookup tables to reciprocals for specified bands
  */
	if (parm.nrbands > 0) {
		...
		for (band = 0; band < parm.nrbands; ++band) {
 /*
  * make sure this band exists
  */
			if (parm.rband[band] >= nbands) {
				error("-r %d: not that many bands",
				      parm.rband[band]);
			}
 /*
  * convert lookup table to reciprocals
  */
			remap = map[parm.rband[band]];
			for (j = len[parm.rband[band]]; --j >= 0;) {
				*remap = (*remap != 0) ? 1 / (*remap) : 1;
				++remap;
			}
		}
	}
