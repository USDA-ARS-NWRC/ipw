...
int *iflg;
{
    ...
    if (indef[1] >= 0 && indef[1] <= maxsys) {
        if (iodef[1] >= 0 && iodef[1] <= maxsys) {
            if (indef[3] >= 0 && indef[3] <= maxunt) {
                ...
                usrerr ("gtrnz0_: Illegal target unit code = %d\n",
                            iodef[3]);
                *iflg = 4;
                return 0;
            }
            usrerr ("gtrnz0_: illegal source unit code = %d\n", 
            indef[3]);
            *iflg = 3;
            return 0;
        }
        usrerr ("gtrnz0_: Illegal target reference system code = %d\n",
                    iodef[1]);
        *iflg = 2;
        return 0;
    }
    usrerr ("gtrnz0_: Illegal source reference system code = %d\n",
                indef[1]);
    *iflg = 1;
    return 0;
  ...
  L80:                          /* CHECK CONSISTENCY BETWEEN UNITS OF
    ...
    if (indef[1] != iodef[1] || indef[2] != iodef[2]) {
        ...
        if (indef[2] > 60 || indef[1] == 1)
            goto L200;
        usrerr ("gtrnz0_: Illegal source zone number = %d\n", 
            indef[2]);
        *iflg = 5;
        return 0;
    }
  ...
  L520:
    if (iodef[2] > 60 || iodef[1] == 1)
        goto L540;
    usrerr ("gtrnz0_: Illegal target zone number = %d\n", iodef[2]);
    *iflg = 6;
    return 0;
