...
double *geog, *proj;
{
  ...
  L120:
    ...
    if (ipfile != NULL) {
        ...
        fprintf(ipfile, "\tSemi-major axis of ellipsoid = %f meters\n", a);
        fprintf(ipfile, "\tEccentricity squared         = %f\n", es);
        fprintf(ipfile, "\tScale at center              = %f\n", ks0);
    }
    if (mode == 0)
        goto L140;
    ...
    if (ipfile != NULL) {
        fprintf(ipfile, "Azimuth of central line        = %s\n", angs2);
        fprintf(ipfile, "Longitude of origin            = %s\n", angs2 + 16);
        fprintf(ipfile, "Latitude of origin             = %s\n", angs2 + 32);
    }
    con = fabs(lat0);
    if (con > epsln && (d_1 = con - halfpi, fabs(d_1)) > epsln)
        goto L160;
    usrerr ("pj20z0_0_: Improper parameter\n");
    *iflg = 2002;
    return 0;
  L140:
    ...
    if (ipfile != NULL) {
        fprintf(ipfile, "\tLongitude of 1st point       = %s\n", angs1);
        fprintf(ipfile, "\tLatitude of 1st point        = %s\n", angs1 + 16);
        fprintf(ipfile, "\tLongitude of 2nd point       = %s\n", angs1 + 32);
        fprintf(ipfile, "\tLatitude of 2nd point        = %s\n", angs1 + 48);
        fprintf(ipfile, "\tLatitude of origin           = %s\n", angs1 + 64);
    }
  ...
  L160:
    ...
    cosalf = cos(alpha);
    if (ipfile != NULL) {
        fprintf(ipfile, "\tFalse easting                = %f meters\n", x0);
        fprintf(ipfile, "\tFalse northing               = %f meters\n", y0);
    }
    switch_ = *zone;
    return 0;
  ...
  L_pf20z0:
    *iflg = 0;
    if (switch_ != 0)
        goto L220;
    usrerr ("pj20z0_0_: Uninitialized transformation\n");
    *iflg = 2000;
    return 0;
  L220:
    sinphi = sin(geog[2]);
  ...
  L250:
    if ((d_1 = fabs(ul) - 1.0, fabs(d_1)) > epsln)
        goto L260;
    usrerr ("pj20z0_0_: Point projects into infinity\n");
    *iflg = 2001;
    return 0;
  L260:
